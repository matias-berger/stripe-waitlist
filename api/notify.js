module.exports = async function handler(req, res) {
        ...
        content: [{
          ...
          value: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px;">
              ...
              <p style="color:#444;line-height:1.6;font-size:14px;margin-top:12px;">Please book a seat before it fills up again.</p>
  ...
  if (id) {
    const patchRes = await fetch(`${SUPABASE_URL}/rest/v1/waitlist?id=eq.${id}`, {
      method: 'PATCH',
      headers: {
        'apikey': SUPABASE_KEY,
    ...
    if (!patchRes.ok) {
      ...
      return res.status(500).json({ error: 'patch failed: ' + txt });
    }
  }
